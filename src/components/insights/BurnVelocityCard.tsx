...
export function BurnVelocityCard({
  burnRateData,
  budget,
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-end mb-2">
          <div className="flex flex-col">
            ...
            <span
              className={`text-xl font-display leading-none tracking-tight ${isSafe ? "text-(--text)" : "text-(--debit)"}`}
            >
              {formatCurrency(projectedTotalSpend)}
            </span>
          </div>
          <div className="text-right">
            <span className="text-[10px] text-(--text-muted) font-medium mb-1 block">
              Monthly Budget
            </span>
        ...
        <div className="h-2 bg-black/5 dark:bg-white/5 rounded-full overflow-hidden w-full mb-3 flex relative">
          <div
            className={`h-full rounded-full transition-all duration-1000 ease-out ${isSafe ? "bg-credit" : "bg-(--debit)"}`}
            style={{ width: `${percentage}%` }}
          />
          {/* Budget line indicator */}
          <div className="absolute top-0 bottom-0 right-0 w-0.5 bg-black/20 dark:bg-white/20 z-10" />
        </div>
        ...
        <div className="p-3 bg-black/4 dark:bg-white/4 rounded-xl border border-black/5 dark:border-white/5">
          {isSafe ? (
            <p className="text-[12px] text-(--text-secondary) m-0 leading-relaxed">
              You're pacing well. If you maintain this velocity, you will finish
              the month safely under budget with roughly{" "}
            ...
            <div className="flex gap-2.5 items-start">
              ...
              />
              <p className="text-[12px] text-(--text-secondary) m-0 leading-relaxed">
                You are currently projected to exceed your budget. At this rate,
                your funds will run out around{" "}
